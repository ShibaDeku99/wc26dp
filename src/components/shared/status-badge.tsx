...
export function StatusBadge({ status, liveMinute, className = '' }: StatusBadgeProps) {
  const config: Record<MatchStatus, { label: string; classes: string }> = {
    ...
    live: {
      label: liveMinute !== undefined ? `● ${liveMinute}'` : '● TRỰC TIẾP',
  ...
  return <Badge variant="outline" className={`${classes} text-[11px] font-semibold tracking-wide uppercase ${className}`}>{label}</Badge>;
}
